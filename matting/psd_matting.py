...
class AutoMattingPSD():
    ...
    def is_crossed(self, cur_draw_type, cur_draw_parameter):
        for frame in self.draw_type_dic.items():
            is_crosse = False
            if ((cur_draw_type == EDrawType.rect or cur_draw_type == EDrawType.min_rect) and (
                    frame[1] == EDrawType.rect or frame[1] == EDrawType.min_rect)):
                is_crosse = self.rect_with_rect_is_corssed(frame[0], cur_draw_parameter)
            elif ((cur_draw_type == EDrawType.rect or cur_draw_type == EDrawType.min_rect) and frame[
                1] == EDrawType.circle):
                is_crosse = self.rect_with_circle_is_corssed(frame[0], cur_draw_parameter)
            elif ((cur_draw_type == EDrawType.rect or cur_draw_type == EDrawType.min_rect) and frame[
                1] == EDrawType.elliptic):
                is_crosse = self.rect_with_elliptic_is_corssed(frame[0], cur_draw_parameter)

            elif ((cur_draw_type == EDrawType.circle) and (
                    frame[1] == EDrawType.rect or frame[1] == EDrawType.min_rect)):
            ...
            elif ((cur_draw_type == EDrawType.elliptic) and (
                    frame[1] == EDrawType.rect or frame[1] == EDrawType.min_rect)):
    ...
    def draw_elliptic(self, size, angle, save_path):
        ...
        resize_img = cv2.resize(img, resize,
                                interpolation=cv2.INTER_LINEAR)  # INTER_NEARAST INTER_LINEAR INTER_AREA INTER_CUBIC INTER_LANCZOS4
    ...
    def rect_with_elliptic_is_corssed(self, elliptic_parameter, cur_rect):
        ...
        ellipse = shapely.affinity.scale(circle, elliptic_parameter[1][0] + 26,
                                         elliptic_parameter[1][1] + 26)  # type(ellipse)=polygon #+26是因为画的线的宽度为26
    ...
    def elliptic_with_elliptic_is_corssed(self, elliptic_parameter, cur_elliptic_parameter):
        ...
        cur_ellipse = shapely.affinity.scale(cur_circle, cur_elliptic_parameter[1][0] + 26, cur_elliptic_parameter[1][
            1] + 26)  # type(ellipse)=polygon #+26是因为画的线的宽度为26
